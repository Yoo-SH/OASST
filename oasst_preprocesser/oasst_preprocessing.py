...
input_file_query_result = conn.execute("SELECT * FROM input_data").fetchdf()

# 매칭된 데이터를 처리하여 새로운 content 생성
for i, row in input_file_query_result.iterrows():   # iterrows는 (인덱스, Series) 쌍을 반환, Series는 각 행의 데이터를 포함.
    ...
    for _, match in matched_rows.iterrows(): # 매칭된 각 행을 다시 iterrows()로 순회. 행의 인덱스는 사용하지 않음.
        input_file_query_result.at[i, 'text'] = remove_company_name(input_file_query_result.at[i, 'text'], match['delete_text']) # input_file_query_result.at[i, 'text']에 이 새로운 문자열을 할당하여 content를 업데이트
...
input_file_query_result.to_excel(output_file_path, index=False)
